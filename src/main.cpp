...
std::string
json_result_val(vp_t& suggestions)
{
  std::string ret;
  ret.reserve(OUTPUT_SIZE_RESERVE);
  for (vp_t::iterator i = suggestions.begin(); i != suggestions.end(); ++i)
...
static void handle_suggest(client_t *client, parsed_url_t &url)
{
  ...
  std::string type = url.path.substr(URL_PREFIX.length());
  DCERR("handle_suggest::q:" << q << ", sn:" << sn << ", callback: " << cb << endl);

  unsigned int n;
  if (sn.empty()) n = NDEFAULT ;
  else
  {
    n = atoi(sn.c_str());
    if (n > NMAX)
...
static void handle_ping(client_t *client, parsed_url_t &url)
{
  ...
  std::string body = uint_to_string(time(NULL));
  write_response(client, 200, "OK", headers, body);
}
...
static void handle_stats(client_t *client, parsed_url_t &url)
{
  ...
  {
    b += sprintf(b, "Data store size: %ld entries\n", pm.repr.size());
  }
...
void serve_request(client_t *client)
{
  ...
  std::string request_uri = url.path.substr(URL_PREFIX.length() - 1);
  DCERR("request_uri: " << request_uri << endl);

  if (request_uri.rfind("-", 0 ) == 0)
  {
